...
import { calc, CR, fmt, fmtSigned, fmtPct, type MapLine } from '@/lib/accounting';
      ...
      <div className="p-5">
        <div className="bg-bg2 border border-border rounded-lg overflow-hidden">
          ...
          {CR.map((l, i) => {
            ...
            const bg = isGtotal ? 'bg-[#0B1F3A]' : isTotal ? 'bg-accent/5' : i % 2 === 0 ? '' : '';
            const fg = isGtotal ? 'text-foreground' : isTotal ? 'text-primary font-semibold' : v < 0 ? 'text-destructive' : '';
            const last = l.id === 'RN_';

            return (
              <div key={l.id} className={`grid grid-cols-[70px_1fr_50px_140px_140px] items-center px-3 py-1.5 border-b border-border/30 text-[11px] ${bg} ${isGtotal || isTotal ? 'font-bold' : ''} ${last ? (rn >= 0 ? 'border-t-2 border-t-success bg-[rgba(52,211,153,.05)]' : 'border-t-2 border-t-destructive bg-[rgba(251,113,133,.05)]') : ''}`}>
                ...
                <span className="text-right font-mono text-fg3">—</span>
              </div>
            );
          })}
        </div>
